...
class MainActivity : ComponentActivity() {
    private companion object {
        const val ActivityNotificationListenerKey = "activity_notification"
    }
    ...
    private fun syncWalletRealtime() {
        if (AuthSession.accessToken.isNullOrBlank()) {
            stopWalletRealtime()
            return
        }
        WalletRealtimeClient.addWalletCreditedListener(ActivityNotificationListenerKey) { payload ->
    ...
    private fun stopWalletRealtime() {
        WalletRealtimeClient.removeWalletCreditedListener(ActivityNotificationListenerKey)
        WalletRealtimeClient.disconnect()
    }
}
